    ...
    return (
        <div className="fixed bottom-6 w-full px-4 z-50 pointer-events-none flex justify-center">
            <div className="w-full max-w-[500px] flex items-end justify-between gap-4 pointer-events-auto">
                {/* Pill Container */}
                <div className="flex bg-white/80 backdrop-blur-md rounded-full shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-gray-100/50 px-2 flex-1 justify-around items-center h-[60px]">
                    {navItems.map((item) => {
                ...
                >
                    ...
                    <Plus className="w-8 h-8 text-primary relative z-10 transition-transform duration-300 group-hover:scale-110" />
                </Link>
            </div>
        </div>
    );
